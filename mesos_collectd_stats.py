...
def configure_callback(conf, is_master, prefix, cluster, instance, path, host,
    ...
    CONFIGS.append({
        'host': host,
        'port': port,
        'mesos_url': "http://" + host + ":" + str(port) + "/monitor/statistics",
...
def parse_stats(conf, json):
    ...
    for name, key in get_stats_string(conf['version']).iteritems():
        for task in json:
            if name not in task['statistics']:
                continue

            try:
                result = float(task['statistics'][name])
            except ValueError:
                result = int(task['statistics'][name])

            dispatch_stat(result, name, task, key, conf)


def dispatch_stat(result, name, task, key, conf):
    ...
    val = collectd.Values(plugin='m')
    val.type = estype
    val.type_instance = ('%s:%s' % (task['executor_id'].split('.')[1], name))
    val.values = [value]
    val.plugin_instance = ('%s' % task['executor_id'].split('.')[0])
